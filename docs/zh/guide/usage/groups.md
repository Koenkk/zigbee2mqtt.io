...
关于如何添加、删除、重命名以及更改组的选项，请参阅 [MQTT 主题与消息中的组章节](./mqtt_topics_and_messages.md#组)
...
payload 应为 `{"group": GROUP, "device": DEVICE}`，其中 `GROUP` 是你要添加/移除设备所属组的 `friendly_name`，`DEVICE` 是你要添加/移除的设备的 `friendly_name`。payload 示例：`{"group":"my_group","device":"my_bulb"}`，响应示例：`{"data":{"device":"my_bulb","endpoint":"default","group":"my_group"},"status":"ok"}`。执行 `remove_all` 时，请求中的 `group` 属性可以省略。
...
控制一个组与控制单个设备类似。例如，要打开组内的所有设备，可以向 `zigbee2mqtt/[GROUP_FRIENDLY_NAME]/set` 发送如下 payload 的 MQTT 消息：
...
如果你想把一个具有多个端点的设备（例如带有 2 个按钮的 QBKG03LM）添加到组中，可以在请求 payload 中指定 `{"group": GROUP, "device": DEVICE, "endpoint": ENDPOINT}`，其中 `ENDPOINT` 是所需的端点名称或 ID。请求 payload 示例：`{"group":"my_group","device":"my_switch","endpoint":"right"}`。
