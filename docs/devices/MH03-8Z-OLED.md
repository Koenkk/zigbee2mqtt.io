---
title: "Manhot MH03-8Z-OLED control via MQTT"
description: "Integrate your Manhot MH03-8Z-OLED via Zigbee2MQTT with whatever smart home infrastructure you are using without the vendor's bridge or gateway."
...
| Exposes | switch (state), countdown_1, countdown_2, countdown_3, countdown_4, countdown_5, countdown_6, countdown_7, countdown_8, child_lock, backlight_switch, backlight_lightness, displayoff_delay, relay_status, light_mode, on_color, off_color, sw1_name, sw2_name, sw3_name, sw4_name, sw5_name, sw6_name, sw7_name, sw8_name, press_on_fun, press_off_fun |
| Picture | ![Manhot MH03-8Z-OLED](https://www.zigbee2mqtt.io/images/devices/MH03-8Z-OLED.png) |
...
### Countdown 7 (numeric)
Timer for switch 7.
Value can be found in the published state on the `countdown_7` property.
It's not possible to read (`/get`) this value.
To write (`/set`) a value publish a message to topic `zigbee2mqtt/FRIENDLY_NAME/set` with payload `{"countdown_7": NEW_VALUE}`.
The minimal value is `0` and the maximum value is `43200`.
The unit of this value is `s`.

### Countdown 8 (numeric)
Timer for switch 8.
Value can be found in the published state on the `countdown_8` property.
It's not possible to read (`/get`) this value.
To write (`/set`) a value publish a message to topic `zigbee2mqtt/FRIENDLY_NAME/set` with payload `{"countdown_8": NEW_VALUE}`.
The minimal value is `0` and the maximum value is `43200`.
The unit of this value is `s`.
...
### Sw7 name (text)
Set switch 7 name.
Value can be found in the published state on the `sw7_name` property.
It's not possible to read (`/get`) this value.
To write (`/set`) a value publish a message to topic `zigbee2mqtt/FRIENDLY_NAME/set` with payload `{"sw7_name": NEW_VALUE}`.

### Sw8 name (text)
Set switch 8 name.
Value can be found in the published state on the `sw8_name` property.
It's not possible to read (`/get`) this value.
To write (`/set`) a value publish a message to topic `zigbee2mqtt/FRIENDLY_NAME/set` with payload `{"sw8_name": NEW_VALUE}`.
...
The possible values are: `disable`, `press_switch_1`, `press_switch_2`, `press_switch_3`, `press_switch_4`, `press_switch_5`, `press_switch_6`, `press_switch_7`, `press_switch_8`.
