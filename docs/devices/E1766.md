...
A `close` or `open` action is triggered when the corresponding button is pressed. If
subsequently held down for > 0.5s, a `stop` action is triggered upon release.
