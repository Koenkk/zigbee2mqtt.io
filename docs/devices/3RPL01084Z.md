...
| Exposes | occupancy, illuminance, volatile_organic_compounds, light (state, brightness, color_xy), effect, power_on_behavior |
...
### Volatile organic compounds (numeric)
Measured VOC value.
Value can be found in the published state on the `volatile_organic_compounds` property.
To read (`/get`) the value publish a message to topic `zigbee2mqtt/FRIENDLY_NAME/get` with payload `{"volatile_organic_compounds": ""}`.
It's not possible to write (`/set`) this value.
