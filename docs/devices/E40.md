...
* `expose_device_state`: Expose device power state as a separate property when enabled. Restart z2m after changing this setting. The value must be `true` or `false`
...
Option to control temperature buttons while child lock is enabled. (Available only in newer version of regulator software)..
Value can be found in the published state on the `temperature_set_enabled_while_blocked` property.
It's not possible to read (`/get`) this value.
To write (`/set`) a value publish a message to topic `zigbee2mqtt/FRIENDLY_NAME/set` with payload `{"temperature_set_enabled_while_blocked": NEW_VALUE}`.
If value equals `ON` enable temperature control during child lock is ON, if `OFF` OFF.
...
Automatically warms the floor every 60 minutes for 7, 11, 15, 19 or 23 minutes according to the level number..
...
### Battery (numeric)
Battery percentage of the paired TRV (can take up to 24 hours after pairing/restarting to appear).
