...
### Color options (composite, top endpoint)
Advanced color behavior.
Can be set by publishing to `zigbee2mqtt/FRIENDLY_NAME/set` with payload `{"color_options_top": {"execute_if_off_top": VALUE}}`
To read (`/get`) the value publish a message to topic `zigbee2mqtt/FRIENDLY_NAME/get` with payload `{"color_options_top": ""}`.
...
### Color options (composite, bottom endpoint)
Advanced color behavior.
Can be set by publishing to `zigbee2mqtt/FRIENDLY_NAME/set` with payload `{"color_options_bottom": {"execute_if_off_bottom": VALUE}}`
To read (`/get`) the value publish a message to topic `zigbee2mqtt/FRIENDLY_NAME/get` with payload `{"color_options_bottom": ""}`.
- `execute_if_off` (binary): Controls whether color and color temperature can be set while light is off allowed values: `true` or `false`
