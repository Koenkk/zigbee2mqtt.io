...
In some cases where users are using a CC2531 USB stick (see https://github.com/Koenkk/zigbee2mqtt/issues/839) performing a shutdown of zigbee2mqtt, removing and reinserting the USB stick, then restarting zigbe2mqtt has proven successful for pairing via the above method when it has not been working.
