...
The minimal value is `0` and the maximum value is `80`.
The unit of this value is `°C`.
...
- `current_heating_setpoint`: Temperature setpoint. To control publish a message to topic `zigbee2mqtt/FRIENDLY_NAME/set` with payload `{"current_heating_setpoint": VALUE}` where `VALUE` is the °C between `5` and `90`. Reading (`/get`) this attribute is not possible.
