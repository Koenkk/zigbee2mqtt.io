...
### No communication after pairing
As proposed by [Heisenberg2980](https://github.com/Heisenberg2980) in [this comment](https://github.com/Koenkk/zigbee2mqtt/issues/9439#issuecomment-956456214) the workaround to this issue is to bind it to the coordinator after pairing by sending the mqtt message `zigbee2mqtt/bridge/request/device/bind` with payload `{"from": "<device friendly name>", "to": "coordinator"}`
