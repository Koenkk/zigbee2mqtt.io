...
|-----|-----|-----|-----|
| Power | Single<br>Double  | Toggle state | Device keeps track of each press alternating between on and off actions.<br>The on action is using `commandOn` command.<br>The off action is using `commandOffWithEffect` command with `{"effectid":0,"effectvariant":0}` payload. |
...
| Hue | Single | Switch between scenes | Device keeps track of each press alternating between two scenes (`"sceneid":1` and `"sceneid":2`) on a pre determined group with `"groupid":43632` using `commandRecall` command. |
