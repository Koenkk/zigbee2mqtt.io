...
| Exposes | battery, action, brightness, linkquality |
...
### Brightness (numeric)
Current Brightness value as an integer between 0-255.
...
It's not possible to read (`/get`) or write (`/set`) this value.
The possible values are: `rotate_left`, `rotate_right`, `rotate_stop`, `play_pause`, `skip_backward`, `skip_forward`.
