...
| Exposes | cover (state, position, tilt), power, energy, motor_state, identify |
...
### Motor state (enum)
This attribute contains two bits which will be set while the motor is active. Thus, devices that do not support positioning or move at a slow pace can still provide feedback. Exposed as motor_state for compatibility..
