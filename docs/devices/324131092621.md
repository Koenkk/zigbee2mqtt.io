...
### Brightness (numeric)
This value keeps track of up-press and down-press. Each time one of these buttons is pressed the brightness value is increased/decreased by 32. This value can be used to set the brightness of ligth devices.
The minimal value is `1` and the maximum value is `255`.

### Duration (numeric)
Time in seconds a button is pressed.

### Counter (numeric)
I am not sure. Should be a counter of number of button presses in a short time.
