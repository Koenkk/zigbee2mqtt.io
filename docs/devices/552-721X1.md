...
| Exposes | switch (state), operation_mode, action_reporting, led_enable, led_color, led_state, led_sync_mode, action |
...
### Led color (enum)
LED ambient color.
Value can be found in the published state on the `led_color` property.
To read (`/get`) the value publish a message to topic `zigbee2mqtt/FRIENDLY_NAME/get` with payload `{"led_color": ""}`.
To write (`/set`) a value publish a message to topic `zigbee2mqtt/FRIENDLY_NAME/set` with payload `{"led_color": NEW_VALUE}`.
