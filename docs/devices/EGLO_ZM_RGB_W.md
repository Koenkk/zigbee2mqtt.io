---
title: "AwoX EGLO_ZM_RGB_W control via MQTT"
description: "Integrate your AwoX EGLO_ZM_RGB_W via Zigbee2MQTT with whatever smart home infrastructure you are using without the vendor's bridge or gateway."
...
| Exposes | light (state, brightness, color_temp, color_xy, color_hs), effect, power_on_behavior |
| Picture | ![AwoX EGLO_ZM_RGB_W](https://www.zigbee2mqtt.io/images/devices/EGLO_ZM_RGB_W.png) |
| White-label | EGLO 113135 |
...
- `color_temp`: To control the color temperature (in reciprocal megakelvin a.k.a. mired scale) publish a message to topic `zigbee2mqtt/FRIENDLY_NAME/set` with payload `{"color_temp": VALUE}` where `VALUE` is a number between `333` and `333`, the higher the warmer the color. To read the color temperature send a message to `zigbee2mqtt/FRIENDLY_NAME/get` with payload `{"color_temp": ""}`. Besides the numeric values the following values are accepted: `coolest`, `warmest`.
