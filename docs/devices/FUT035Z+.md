...
| Picture | ![Miboxer FUT035Z+](https://www.zigbee2mqtt.io/images/devices/FUT035ZPlus.jpg) |
...
### Pairing
The device will enter pairing mode when turned on, if it has not been paired previously. Pairing mode can be forced by long-pressing the SET button until the light starts flashing.

The lights connected to the controller will fade in and out when the controlled is in pairing mode.

To switch the device between single color mode (red indicator light) and dual white mode (green light indicator) press the SET button multiple times, until the desired output mode has been found. 
