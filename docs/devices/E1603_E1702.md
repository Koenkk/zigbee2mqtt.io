...
This switch also supports the "On With Timed Off" command. It allows the switch to turn on for a specified duration and then turn off automatically. For example to turn on for five minutes, use the payload `{"state": "on", "on_time": 300}`. Durations are specified in seconds and fractions are supported at up to 1/10th of a seconds. See chapter 3.8.2.3.6 of the ZCL for more advanced scenarios.  
