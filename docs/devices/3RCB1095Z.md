---
title: "Third Reality 3RCB1095Z control via MQTT"
description: "Integrate your Third Reality 3RCB1095Z via Zigbee2MQTT with whatever smart home infrastructure you are using without the vendor's bridge or gateway."
...
# Third Reality 3RCB1095Z

|     |     |
|-----|-----|
| Model | 3RCB1095Z  |
| Vendor  | [Third Reality](/supported-devices/#v=Third%20Reality)  |
| Description | Smart Color Bulb ZL2 |
| Exposes | light (state, brightness, color_temp, color_temp_startup, color_xy, color_hs), effect, power_on_behavior, start_bind |
| Picture | ![Third Reality 3RCB1095Z](https://www.zigbee2mqtt.io/images/devices/3RCB1095Z.png) |
...
### Start bind (enum)
Start bind the light to the controller.
Value can be found in the published state on the `start_bind` property.
To read (`/get`) the value publish a message to topic `zigbee2mqtt/FRIENDLY_NAME/get` with payload `{"start_bind": ""}`.
To write (`/set`) a value publish a message to topic `zigbee2mqtt/FRIENDLY_NAME/set` with payload `{"start_bind": NEW_VALUE}`.
The possible values are: `StartBind`.
