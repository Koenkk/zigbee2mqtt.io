...
### Climate 
This climate device supports the following features: `local_temperature`, `occupied_heating_setpoint`, `system_mode`, `pi_heating_demand`, `programming_operation_mode`, `running_state`.
...
If value equals `Heat Available` heat available is ON, if `No Heat Available` OFF.
...
It's not possible to write (`/set`) this value.
If value equals `Heat Request` heat required is ON, if `No Heat Request` OFF.
...
If value equals `Room Sensor Mode` radiator covered is ON, if `Auto Offset Mode` OFF.
...
If value equals `Open` window open external is ON, if `Closed` OFF.
