...
| Exposes | system_status_code, climate (local_temperature, occupied_heating_setpoint, system_mode, pi_heating_demand, running_state), max_heat_setpoint_limit, programming_operation_mode, occupied_heating_setpoint_scheduled, abs_max_heat_setpoint_limit, battery, keypad_lockout, mounted_mode_active, mounted_mode_control, thermostat_orientation, viewing_direction, heat_available, heat_required, setpoint_change_source, external_measured_room_sensor, radiator_covered, window_open_feature, window_open_internal, window_open_external, day_of_week, trigger_time, algorithm_scale_factor, load_balancing_enable, load_room_mean, load_estimate, preheat_status, preheat_time, adaptation_run_status, adaptation_run_settings, adaptation_run_control, regulation_setpoint_offset |
...
### Pairing
* Insert the batteries. Push the "o" button for 3 seconds until M display. Install the device on the valve.
* Push the "o" button for briefly to enter Pairing mode.
* Add the device in Zigbee2MQTT
* Press and hold the "o" button for 3 seconds. The device is now installed and will display the current temperature.
...
### Version number issue after OTA

When updating the firmware using OTA, you might end up with a version number such as `AAAABBBBCCCCDDDD`. To resolve this, just re-interview the device (by pressing the blue `i` icon on the device page).
...
Diagnostic error codes (e.g. 'invalid_clock_information,low_battery'). 'ok' when no errors..
Value can be found in the published state on the `system_status_code` property.
To read (`/get`) the value publish a message to topic `zigbee2mqtt/FRIENDLY_NAME/get` with payload `{"system_status_code": ""}`.
It's not possible to write (`/set`) this value.

### Climate 
This climate device supports the following features: `local_temperature`, `occupied_heating_setpoint`, `system_mode`, `pi_heating_demand`, `running_state`.
...
- `running_state`: Running state based on danfossOutputStatus and danfossHeatRequired. Possible values are: `idle`, `heat`. To read send a message to `zigbee2mqtt/FRIENDLY_NAME/get` with payload `{"running_state": ""}`.
...
Absolute Maximum Heating Setpoint Limit for the device. Adjust the 'Max heat setpoint limit' accordingly..
...
### Thermostat orientation (binary)
Thermostat Orientation. This is important for the PID in how it assesses temperature..
Value can be found in the published state on the `thermostat_orientation` property.
To read (`/get`) the value publish a message to topic `zigbee2mqtt/FRIENDLY_NAME/get` with payload `{"thermostat_orientation": ""}`.
To write (`/set`) a value publish a message to topic `zigbee2mqtt/FRIENDLY_NAME/set` with payload `{"thermostat_orientation": NEW_VALUE}`.
If value equals `vertical` thermostat orientation is ON, if `horizontal` OFF.

### Viewing direction (binary)
Viewing/display direction.
...
If value equals `upside-down` viewing direction is ON, if `normal` OFF.

### Heat available (binary)
Not clear how this affects operation. However, it would appear that the device does not execute any motor functions if this is set to false. This may be a means to conserve battery during periods that the heating system is not energized (e.g. during summer)..
...
Whether or not the unit needs warm water..
...
Set if the window is open or closed. This setting will trigger a change in the internal window and heating demand..
...
### Preheat time (numeric)
Timestamp of the scheduled setpoint currently being preheated to (read-only).
Value can be found in the published state on the `preheat_time` property.
To read (`/get`) the value publish a message to topic `zigbee2mqtt/FRIENDLY_NAME/get` with payload `{"preheat_time": ""}`.
It's not possible to write (`/set`) this value.
...
It's not possible to write (`/set`) this value.
The possible values are: `none`, `in_progress`, `found`, `lost`, `lost_in_progress`.
...
The possible values are: `idle`, `initiate_adaptation`, `cancel_adaptation`.

### Regulation setpoint offset (numeric)
Regulation SetPoint Offset in range -2.5°C to 2.5°C in steps of 0.1°C..
