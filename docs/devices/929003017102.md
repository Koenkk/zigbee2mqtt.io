...
It's possible to configure this switch to directly control Zigbee groups. In this way, the switch can control e.g. a group of light bulbs even when Zigbee2MQTT is not running, and it can also make controlling the lights feel a bit more responsive. This switch can also act as a dimmer: for each successive long press of the switch, it will alternate between increasing and decreasing brightness. 

- RDM001 modules on `v1.x.x` firmware send the *Toggle* command  
⤷ All group members will individually toggle  
- RDM001 modules on `v2.x.x` firmware and all RDM004 modules alternate between *On* and *OffWithEffect*  
⤷ It's possible the button press will send On when the light is already On

To set this all up, follow these steps in the Zigbee2MQTT frontend:
