...
It's not possible to read (`/get`) or write (`/set`) this value.
The possible values are: `color_move`, `color_temperature_move`, `hue_move`, `hue_stop`, `brightness_step_up`, `brightness_step_down`, `brightness_stop`, `brightness_move_up`, `brightness_move_down` `recall_*`, `on`, `off`.
