---
title: "Yandex YNDX_00530 control via MQTT"
description: "Integrate your Yandex YNDX_00530 via Zigbee2MQTT with whatever smart home infrastructure you are using without the vendor's bridge or gateway."
...
| Description | Dimmer |
| Exposes | light (state, brightness), effect, power_on_behavior, ballast_minimum_level, ballast_maximum_level, led_indicator, button_mode |
| Picture | ![Yandex YNDX_00530](https://www.zigbee2mqtt.io/images/devices/YNDX_00530.png) |
...
### Button mode (enum)
Dimmer button mode.
Value can be found in the published state on the `button_mode` property.
To read (`/get`) the value publish a message to topic `zigbee2mqtt/FRIENDLY_NAME/get` with payload `{"button_mode": ""}`.
To write (`/set`) a value publish a message to topic `zigbee2mqtt/FRIENDLY_NAME/set` with payload `{"button_mode": NEW_VALUE}`.
The possible values are: `general`, `alternative`.
