---
title: "EFEKTA EFEKTA_Air_Quality_Station_2o control via MQTT"
description: "Integrate your EFEKTA EFEKTA_Air_Quality_Station_2o via Zigbee2MQTT with whatever smart home infrastructure you are using without the vendor's bridge or gateway."
...
| Exposes | co2, pm1, pm25, pm4, pm10, pm_size, pm_n05, pm_n1, pm_n25, pm_n4, pm_n10, voc_index, nox_index, ozone, temperature, humidity, illuminance, report_delay, auto_brightness, night_onoff_backlight, night_on_backlight, night_off_backlight, temperature_offset, humidity_offset, ozone_offset, auto_clean_interval, manual_clean, set_altitude, forced_recalibration, manual_forced_recalibration, automatic_self_calibration, factory_reset_co2 |
| Picture | ![EFEKTA EFEKTA_Air_Quality_Station_2o](https://www.zigbee2mqtt.io/images/devices/EFEKTA_Air_Quality_Station_2o.png) |
...
### Pm n05 (numeric)
Measured PM0.5 (particulate matter) number concentration.
Value can be found in the published state on the `pm_n05` property.
It's not possible to read (`/get`) or write (`/set`) this value.
The unit of this value is `#/cm³`.

### Pm n1 (numeric)
Measured PM1.0 (particulate matter) number concentration.
Value can be found in the published state on the `pm_n1` property.
It's not possible to read (`/get`) or write (`/set`) this value.
The unit of this value is `#/cm³`.

### Pm n25 (numeric)
Measured PM2.5 (particulate matter) number concentration.
Value can be found in the published state on the `pm_n25` property.
It's not possible to read (`/get`) or write (`/set`) this value.
The unit of this value is `#/cm³`.

### Pm n4 (numeric)
Measured PM4.0 (particulate matter) number concentration.
Value can be found in the published state on the `pm_n4` property.
It's not possible to read (`/get`) or write (`/set`) this value.
The unit of this value is `#/cm³`.

### Pm n10 (numeric)
Measured PM10.0 (particulate matter) number concentration.
Value can be found in the published state on the `pm_n10` property.
It's not possible to read (`/get`) or write (`/set`) this value.
The unit of this value is `#/cm³`.
...
### Nox index (numeric)
NOx index.
Value can be found in the published state on the `nox_index` property.
It's not possible to read (`/get`) or write (`/set`) this value.
The unit of this value is `NOx Index points`.

### Ozone (numeric)
Ozone concentration.
Value can be found in the published state on the `ozone` property.
It's not possible to read (`/get`) or write (`/set`) this value.
...
### Ozone offset (numeric)
Adjust ozone.
Value can be found in the published state on the `ozone_offset` property.
It's not possible to read (`/get`) this value.
To write (`/set`) a value publish a message to topic `zigbee2mqtt/FRIENDLY_NAME/set` with payload `{"ozone_offset": NEW_VALUE}`.
