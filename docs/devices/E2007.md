...
To read (`/get`) the value publish a message to topic `zigbee2mqtt/FRIENDLY_NAME/get` with payload `{"replace_filter": "false"}`.
To write (`/set`) the value publish a message to topic `zigbee2mqtt/FRIENDLY_NAME/get` with payload `{"replace_filter": "false"}`.
...
To write (`/set`) the value publish a message to topic `zigbee2mqtt/FRIENDLY_NAME/get` with payload `{"filter_age": "100"}`.
