...
| Exposes | local_temperature, lock (state), power, energy, climate (occupied_heating_setpoint, local_temperature, system_mode, running_state, pi_heating_demand), linkquality |
...
### Backlight
You can choose if you want to have the backlight always powered on, or only on demand (when you press a button). <br>
To control the backlight, publish a message to topic `zigbee2mqtt/FRIENDLY_NAME/set/backlight_auto_dim` with payload `0` for "on demand", or `1` for "always on".
