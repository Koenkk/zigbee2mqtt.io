---
title: "Philips 929004608003 control via MQTT"
description: "Integrate your Philips 929004608003 via Zigbee2MQTT with whatever smart home infrastructure you are using without the vendor's bridge or gateway."
...
# Philips 929004608003

|     |     |
|-----|-----|
| Model | 929004608003  |
| Vendor  | [Philips](/supported-devices/#v=Philips)  |
| Description | Hue OmniGlow lightstrip AU (3m) |
| Exposes | light (state, brightness, color_temp, color_temp_startup, color_xy, color_hs), power_on_behavior, gradient_scene, gradient, effect, effect_speed, effect_color, gradient_style, gradient_scale, gradient_offset, identify |
| Picture | ![Philips 929004608003](https://www.zigbee2mqtt.io/images/devices/929004608003.png) |
...
The possible values are: `blink`, `breathe`, `okay`, `channel_change`, `candle`, `fireplace`, `colorloop`, `sparkle`, `opal`, `glisten`, `prism`, `underwater`, `cosmos`, `sunbeam`, `enchant`, `sunset`, `sunrise`, `sparkle`, `opal`, `glisten`, `underwater`, `cosmos`, `sunbeam`, `enchant`, `none`, `finish_effect`, `stop_effect`, `stop_hue_effect`.
...
Value will **not** be published in the state.
It's not possible to read (`/get`) this value.
To write (`/set`) a value publish a message to topic `zigbee2mqtt/FRIENDLY_NAME/set` with payload `{"effect_color": NEW_VALUE}`.

### Gradient style (enum)
Gradient rendering style: linear (smooth blend), scattered (color per segment), mirrored (symmetric from center).
Value can be found in the published state on the `gradient_style` property.
To read (`/get`) the value publish a message to topic `zigbee2mqtt/FRIENDLY_NAME/get` with payload `{"gradient_style": ""}`.
To write (`/set`) a value publish a message to topic `zigbee2mqtt/FRIENDLY_NAME/set` with payload `{"gradient_style": NEW_VALUE}`.
The possible values are: `linear`, `scattered`, `mirrored`.

### Gradient scale (numeric)
Gradient scale (0=auto fit, 1.0+=number of colors visible).
Value will **not** be published in the state.
It's not possible to read (`/get`) this value.
To write (`/set`) a value publish a message to topic `zigbee2mqtt/FRIENDLY_NAME/set` with payload `{"gradient_scale": NEW_VALUE}`.
The minimal value is `0` and the maximum value is `31`.

### Gradient offset (numeric)
Gradient color offset (0=start from first color).
Value will **not** be published in the state.
It's not possible to read (`/get`) this value.
To write (`/set`) a value publish a message to topic `zigbee2mqtt/FRIENDLY_NAME/set` with payload `{"gradient_offset": NEW_VALUE}`.
The minimal value is `0` and the maximum value is `31`.
