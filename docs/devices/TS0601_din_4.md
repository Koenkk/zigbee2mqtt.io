...
## Notes
This device can transmit multiple LQI updates per second which might overwhelm the Zigbee network.

### Pairing
Press and hold 3rd button from left for +/- 5 seconds, the WiFi led (2nd from left) should start short blinks once/second.

### Buttons
Press and hold 1st button from left for a few seconds to start modifying various settings: "over-voltage", "under-voltage", "leak-threshold", "over-current-threshold", "recover-time", "prepaying credits", etc. 
Use the 1st button to cycle through the settings. Use 2nd and 3rd button to increase/decrease values.
