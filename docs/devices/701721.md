...
### Supported features
The features related to control by an external temperature sensor, i.e. `external_measured_room_sensor` and `radiator_covered`, do not work with the stock Popp firmware `01.12.0008 01.12`, dated `20220524`.

They do work with the Danfoss firmware `01.18.0008 01.18`, dated `20220121`, that can be installed onto the device via OTA. But that firmware is called "not released" by the Popp technical support.
