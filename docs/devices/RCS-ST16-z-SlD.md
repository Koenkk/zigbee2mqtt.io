---
title: "Slacky-DIY RCS-ST16-z-SlD control via MQTT"
description: "Integrate your Slacky-DIY RCS-ST16-z-SlD via Zigbee2MQTT with whatever smart home infrastructure you are using without the vendor's bridge or gateway."
...
| Exposes | switch (state), power_on_behavior, key_lock, led_control, power, voltage, current, energy, energy_reset, protect_control, automatic_restart, voltage_min, voltage_max, current_max, power_max, time_reload |
| Picture | ![Slacky-DIY RCS-ST16-z-SlD](https://www.zigbee2mqtt.io/images/devices/RCS-ST16-z-SlD.png) |



<!-- Notes BEGIN: You can edit here. Add "## Notes" headline if not already present. -->
[Original project description](https://github.com/slacky1965/socket_ekf_stockholm_zrd)
...
Value can be found in the published state on the `key_lock` property.
To read (`/get`) the value publish a message to topic `zigbee2mqtt/FRIENDLY_NAME/get` with payload `{"key_lock": ""}`.
To write (`/set`) a value publish a message to topic `zigbee2mqtt/FRIENDLY_NAME/set` with payload `{"key_lock": NEW_VALUE}`.
If value equals `LOCK` key lock is ON, if `UNLOCK` OFF.
...
The possible values are: `off`, `on`, `on/off`, `off/on`.
