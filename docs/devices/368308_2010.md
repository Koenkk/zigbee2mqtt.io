...
- `local_temperature_calibration`: Temperature offset. To control publish a message to topic `zigbee2mqtt/FRIENDLY_NAME/set` with payload `{"local_temperature_calibration": VALUE}` where `VALUE` is the °C between `-12.5` and `5.5`. To read send a message to `zigbee2mqtt/FRIENDLY_NAME/get` with payload `{"local_temperature_calibration": ""}`.
