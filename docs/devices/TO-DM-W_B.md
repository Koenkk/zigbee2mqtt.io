---
title: "Nova Digital TO-DM-W/B control via MQTT"
description: "Integrate your Nova Digital TO-DM-W/B via Zigbee2MQTT with whatever smart home infrastructure you are using without the vendor's bridge or gateway."
...
| Exposes | light (state, brightness), brightness_min, brightness_max, light_type, power_on_behavior, indicator_mode, backlight_switch, countdown |
| Picture | ![Nova Digital TO-DM-W/B](https://www.zigbee2mqtt.io/images/devices/TO-DM-W-B.png) |
...
Value can be found in the published state on the `brightness_min` property.
It's not possible to read (`/get`) this value.
To write (`/set`) a value publish a message to topic `zigbee2mqtt/FRIENDLY_NAME/set` with payload `{"brightness_min": NEW_VALUE}`.
The minimal value is `1` and the maximum value is `100`.
The unit of this value is `%`.

### Brightness max (numeric)
Maximum brightness limit.
Value can be found in the published state on the `brightness_max` property.
It's not possible to read (`/get`) this value.
To write (`/set`) a value publish a message to topic `zigbee2mqtt/FRIENDLY_NAME/set` with payload `{"brightness_max": NEW_VALUE}`.
The minimal value is `1` and the maximum value is `100`.
The unit of this value is `%`.

### Light type (enum)
Type of connected light load.
