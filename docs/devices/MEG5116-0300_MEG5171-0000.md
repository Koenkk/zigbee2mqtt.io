...
| Exposes | light (state, brightness, level_config), ballast_minimum_level, ballast_maximum_level, dimmer_mode, linkquality, indicator_mode, switch_action |
| Picture | ![Schneider Electric MEG5116-0300/MEG5171-0000](https://www.zigbee2mqtt.io/images/devices/MEG5116-0300.png)                                      |
...
### Switch action (enum)
Button Action.
Value can be found in the published state on the `switch_action` property.
To read (`/get`) the value publish a message to topic `zigbee2mqtt/FRIENDLY_NAME/get` with payload `{"switch_action": ""}`.
To write (`/set`) a value publish a message to topic `zigbee2mqtt/FRIENDLY_NAME/set` with payload `{"switch_action": NEW_VALUE}`.
The possible values are: `light`, `light_opposite`, `dimmer`, `dimmer_opposite`, `standard_shutter`,
`standard_shutter_opposite`, `schneider_shutter`, `schneider_shutter_opposite`, `scene`, `toggle_light`, `toggle_dimmer`,
`alternate_light`, `alternate_dimmer`, `not_used`.
