...
| Exposes | produced_energy, ct_direction, power, voltage, ac_frequency, power_factor, current, energy, temperature |
| Picture | ![ShinaSystem PMM-300Z2](https://www.zigbee2mqtt.io/images/devices/PMM-300Z2.png) |
...
### Ct direction (enum)
Auto (Default): All measured power and energy values are treated as positive regardless of CT installation direction, And there is only energy consumption, not produced energy. And Manual additionally displays produced energy(e.g. when solar power is installed, set it manually). And it displays energy consumption and production according to the manual forward/reverse settings..
Value can be found in the published state on the `ct_direction` property.
To read (`/get`) the value publish a message to topic `zigbee2mqtt/FRIENDLY_NAME/get` with payload `{"ct_direction": ""}`.
To write (`/set`) a value publish a message to topic `zigbee2mqtt/FRIENDLY_NAME/set` with payload `{"ct_direction": NEW_VALUE}`.
The possible values are: `Auto`, `Manual(Forward)`, `Manual(Reverse)`.
