...
- `local_temperature`: Current temperature used by the thermostat (in °C). To read send a message to `zigbee2mqtt/FRIENDLY_NAME/get` with payload `{"local_temperature": ""}`.
...
### Internal sensor temperature (numeric)
Temperature measured by the thermostat's internal sensor.
Value can be found in the published state on the `local_temperature_internal` property.
It's not possible to read (`/get`) or write (`/set`) this value.
