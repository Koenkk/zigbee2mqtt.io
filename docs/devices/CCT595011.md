...
description: "Integrate your Schneider Electric CCT595011 via Zigbee2MQTT with whatever smart home infrastructure you are using without the vendors bridge or gateway."
addedAt: 
...
| Exposes | battery, occupancy, illuminance_lux, illuminance, sensitivity_levels, sensitivity, linkquality |
...
### Sensitivity_levels (numeric)
Value can be found in the published state on the `sensitivity_levels` property.
It's not possible to read (`/get`) or write (`/set`) this value.
