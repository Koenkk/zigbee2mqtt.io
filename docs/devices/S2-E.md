---
title: "Heiman S2-E control via MQTT"
description: "Integrate your Heiman S2-E via Zigbee2MQTT with whatever smart home infrastructure you are using without the vendor's bridge or gateway."
...
| Exposes | battery, identify, temperature, smoke, battery_low, test, alarm_state, fault_state, muted, trigger_selftest, temporary_mute, heartbeat_indicator, interconnectable, smoke_level, smoke_unit, chamber_contamination, link_available, siren_for_automation_only, shielding_sensor_detection, temperature_offset, reported_packages, rejoin_count, reboot_count |
| Picture | ![Heiman S2-E](https://www.zigbee2mqtt.io/images/devices/S2-E.png) |
...
It's not possible to write (`/set`) this value.
The possible values are: `normal`, `prealarm`, `alarmed`, `critical_alarm`.
...
### Shielding sensor detection (numeric)
Duration of shielding sensor detection.
Value can be found in the published state on the `shielding_sensor_detection` property.
To read (`/get`) the value publish a message to topic `zigbee2mqtt/FRIENDLY_NAME/get` with payload `{"shielding_sensor_detection": ""}`.
To write (`/set`) a value publish a message to topic `zigbee2mqtt/FRIENDLY_NAME/set` with payload `{"shielding_sensor_detection": NEW_VALUE}`.
The minimal value is `0` and the maximum value is `20`.
