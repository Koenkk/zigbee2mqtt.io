---
title: "Slacky-DIY THERM_SLACKY_DIY_R0D control via MQTT"
description: "Integrate your Slacky-DIY THERM_SLACKY_DIY_R0D via Zigbee2MQTT with whatever smart home infrastructure you are using without the vendor's bridge or gateway."
...
| Exposes | child_lock, brightness_level, screen_sleep_time, led_indicator, programming_operation_mode, hysteresis_temperature, min_heat_setpoint_limit, max_heat_setpoint_limit, frost_protect, heat_protect, eco_mode_cool_temperature, sensor, outdoor_temperature, climate (local_temperature, occupied_heating_setpoint, local_temperature_calibration, system_mode, running_state, schedule), schedule_monday, schedule_tuesday, schedule_wednesday, schedule_thursday, schedule_friday, schedule_saturday, schedule_sunday, settings_reset |
| Picture | ![Slacky-DIY THERM_SLACKY_DIY_R0D](https://www.zigbee2mqtt.io/images/devices/THERM_SLACKY_DIY_R0D.png) |
...
### Screen sleep time (enum)
Screen sleep timeout.
Value can be found in the published state on the `screen_sleep_time` property.
To read (`/get`) the value publish a message to topic `zigbee2mqtt/FRIENDLY_NAME/get` with payload `{"screen_sleep_time": ""}`.
To write (`/set`) a value publish a message to topic `zigbee2mqtt/FRIENDLY_NAME/set` with payload `{"screen_sleep_time": NEW_VALUE}`.
The possible values are: `10s`, `20s`, `30s`, `40s`, `50s`, `60s`.

### Led indicator (binary)
Enables/disables light ring indicator.
