---
title: "Slacky-DIY THERM_SLACKY_DIY_R0B control via MQTT"
description: "Integrate your Slacky-DIY THERM_SLACKY_DIY_R0B via Zigbee2MQTT with whatever smart home infrastructure you are using without the vendor's bridge or gateway."
...
| Exposes | child_lock, programming_operation_mode, sensor, screen_sleep_mode, hysteresis_temperature, min_heat_setpoint_limit, max_heat_setpoint_limit, climate (local_temperature, occupied_heating_setpoint, local_temperature_calibration, system_mode, running_state, schedule), schedule_monday, schedule_saturday, schedule_sunday |
| Picture | ![Slacky-DIY THERM_SLACKY_DIY_R0B](https://www.zigbee2mqtt.io/images/devices/THERM_SLACKY_DIY_R0B.png) |
...
The possible values are: `Inner (IN)`, `Outer (OU)`.

### Screen sleep mode (enum)
Screen sleep mode of brightness.
Value can be found in the published state on the `screen_sleep_mode` property.
To read (`/get`) the value publish a message to topic `zigbee2mqtt/FRIENDLY_NAME/get` with payload `{"screen_sleep_mode": ""}`.
To write (`/set`) a value publish a message to topic `zigbee2mqtt/FRIENDLY_NAME/set` with payload `{"screen_sleep_mode": NEW_VALUE}`.
The possible values are: `Off`, `Dim`, `On`.
...
The minimal value is `35` and the maximum value is `50`.
The unit of this value is `°C`.
