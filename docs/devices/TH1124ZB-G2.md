---
title: "Sinopé TH1124ZB-G2 control via MQTT"
description: "Integrate your Sinopé TH1124ZB-G2 via Zigbee2MQTT with whatever smart home infrastructure you are using without the vendor's bridge or gateway."
...
- **ecoMode** : off=-128, range: -100 to 100, amount of setpoint reduction/increase for peak and pre-heating, 0 only for the eco icon to flash
- **ecoMode1** : off=255, range: 0 to 99 Set pi_heating_demand percentage limit 0% to 99% (255 = 100%)
- **EcoMode2** : off=255, range: 0 to 100, set heating restart if room temperature goes x% below setpoint
...
If you want to enable the eco mode using a script (python in this example), you will need to send an MQTT message formatted like this:
``` python
topic = "zigbee2mqtt/<FRIENDLY_NAME>/1/set"
payload = {"write": {"cluster": "manuSpecificSinope", "options": {}, "payload": {"ecoMode": <VALUE>}}}
```
...
The possible values are: `on_demand`, `sensing`, `off`.
...
### Eco mode (numeric)
Adjusts temperature setpoint to fulfill demand response.
