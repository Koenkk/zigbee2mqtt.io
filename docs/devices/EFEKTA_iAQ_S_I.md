---
title: "EFEKTA EFEKTA_iAQ_S_I control via MQTT"
description: "Integrate your EFEKTA EFEKTA_iAQ_S_I via Zigbee2MQTT with whatever smart home infrastructure you are using without the vendor's bridge or gateway."
...
Value can be found in the published state on the `th_sensor` property.
It's not possible to read (`/get`) this value.
To write (`/set`) a value publish a message to topic `zigbee2mqtt/FRIENDLY_NAME/set` with payload `{"th_sensor": NEW_VALUE}`.
