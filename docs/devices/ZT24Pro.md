---
title: "Tuya ZT24Pro control via MQTT"
description: "Integrate your Tuya ZT24Pro via Zigbee2MQTT with whatever smart home infrastructure you are using without the vendor's bridge or gateway."
...
| Exposes | presence, distance, illuminance, temperature, humidity, move_sensitivity, presence_sensitivity, presence_timeout, detection_distance_max, temperature_correction, humidity_correction, state, living_room, bedroom, bathroom, sleep, radar_switch |
| Picture | ![Tuya ZT24Pro](https://www.zigbee2mqtt.io/images/devices/ZT24Pro.png) |
...
The minimal value is `-1.5` and the maximum value is `1.5`.
The unit of this value is `℃`.

### Humidity correction (numeric)
humidity_correction.
Value can be found in the published state on the `humidity_correction` property.
It's not possible to read (`/get`) this value.
To write (`/set`) a value publish a message to topic `zigbee2mqtt/FRIENDLY_NAME/set` with payload `{"humidity_correction": NEW_VALUE}`.
