...
More information can be found in the [ubisys D1 technical reference manual](https://www.ubisys.de/downloads/ubisys-d1-technical-reference.pdf), chapter "7.2.8. Dimmer Setup Cluster (Server)".
...
### Identify (enum, default endpoint)
Initiate device identification.
Value will **not** be published in the state.
It's not possible to read (`/get`) this value.
To write (`/set`) a value publish a message to topic `zigbee2mqtt/FRIENDLY_NAME/set` with payload `{"identify_default": NEW_VALUE}`.
