...
For the OTA update to work on this device you need to keep pressing the button on the device again and again until the update is complete, so the device doesn´t loose the connection with zigbee2mqtt.
