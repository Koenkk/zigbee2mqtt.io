...
| Exposes | switch (state), battery, voltage, battery_low, status, identify, kamikaze, stall_time, battery_type, end_lag, offline_action, inverted |
...
It's not possible to write (`/set`) this value.
The possible values are: `OFF`, `ON`, `MOVING`, `STUCK`, `OFFLINE_CLOSE`, `OFFLINE_OPEN`, `BLOCKED`.
...
### Offline action (enum)
What the valve does when the coordinator stops answering. The RESTORE variants return to the previous position once it answers again.
Value can be found in the published state on the `offline_action` property.
To read (`/get`) the value publish a message to topic `zigbee2mqtt/FRIENDLY_NAME/get` with payload `{"offline_action": ""}`.
To write (`/set`) a value publish a message to topic `zigbee2mqtt/FRIENDLY_NAME/set` with payload `{"offline_action": NEW_VALUE}`.
The possible values are: `NONE`, `CLOSE_RESTORE`, `OPEN_RESTORE`, `CLOSE`, `OPEN`.

### Inverted (binary)
Swap the two valve positions, for actuators fitted the other way round. Changing this does not move the valve, it only changes which position is reported as on and which command drives where.
Value can be found in the published state on the `inverted` property.
To read (`/get`) the value publish a message to topic `zigbee2mqtt/FRIENDLY_NAME/get` with payload `{"inverted": ""}`.
To write (`/set`) a value publish a message to topic `zigbee2mqtt/FRIENDLY_NAME/set` with payload `{"inverted": NEW_VALUE}`.
If value equals `ON` inverted is ON, if `OFF` OFF.
