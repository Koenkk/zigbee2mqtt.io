...
| Exposes | switch (state), power, voltage, ac_frequency, current, energy, led_mode, led_colors, led_power_brightness, led_night_mode, buttons_enabled, power_on_behavior, wifi_status, ip_address, dhcp_enabled, wifi_config, identify |
...
### LED Mode (enum)
Controls the behaviour of the LED rings around the sockets. The device cannot report this back over Zigbee, so it shows what was last set from here.
...
Colors of the LED rings while the LED mode is 'switch'. The device cannot report this back over Zigbee, so it shows what was last set from here.
...
Brightness of the LED rings while the LED mode is 'power'. The device cannot report this back over Zigbee, so it shows what was last set from here.
...
Adjust LED brightness during night time. The device cannot report this back over Zigbee, so it shows what was last set from here.
...
Whether each socket button switches its own socket or is detached. The device cannot report this back over Zigbee, so it shows what was last set from here.
