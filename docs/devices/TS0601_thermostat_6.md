---
title: "Tuya TS0601_thermostat_6 control via MQTT"
description: "Integrate your Tuya TS0601_thermostat_6 via Zigbee2MQTT with whatever smart home infrastructure you are using without the vendor's bridge or gateway."
...
| Exposes | child_lock, battery, open_window, switch (state), battery_low, climate (current_heating_setpoint, local_temperature, preset, system_mode, local_temperature_calibration), schedule_monday, schedule_tuesday, schedule_wednesday, schedule_thursday, schedule_friday, schedule_saturday, schedule_sunday, comfort_temperature, eco_temperature, scale_protection, frost_protection, error |
| Picture | ![Tuya TS0601_thermostat_6](https://www.zigbee2mqtt.io/images/devices/TS0601_thermostat_6.png) |
...
- `preset`: Mode of this device (similar to system_mode). To control publish a message to topic `zigbee2mqtt/FRIENDLY_NAME/set` with payload `{"preset": VALUE}` where `VALUE` is one of: `manual`, `schedule`, `eco`, `comfort`, `antifrost`, `off`. Reading (`/get`) this attribute is not possible.
