...
### Time (numeric) (dial only)
Raw value that represent the amount the dial was turned.
Value can be found in the pusbished state on the `time` property.
It's not possible to read (`/get`) or write (`/set`) this value.
The minimal value is `0` and the maximum value is `255`.
One step amount to `15`.

### Direction (enum) (dial only)
...
### Type (enum) (dial only)
Type of the rotation. First message reports as `step` and consuquest messages reports `rotate` value.
Value can be found in the published state on the `type` property.
It's not possible to read (`/get`) or write (`/set`) this value.
Possible values are: `step` and `rotate`.
