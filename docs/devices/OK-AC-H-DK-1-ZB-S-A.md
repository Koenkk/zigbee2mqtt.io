---
title: "ONOKOM OK-AC-H-DK-1-ZB-S-A control via MQTT"
description: "Integrate your ONOKOM OK-AC-H-DK-1-ZB-S-A via Zigbee2MQTT with whatever smart home infrastructure you are using without the vendor's bridge or gateway."
...
| Picture | ![ONOKOM OK-AC-H-DK-1-ZB-S-A](https://www.zigbee2mqtt.io/images/devices/OK-AC-H-DK-1-ZB-S-A.png) |
...
Screen low bright.
Value can be found in the published state on the `screen_low_bright` property.
To read (`/get`) the value publish a message to topic `zigbee2mqtt/FRIENDLY_NAME/get` with payload `{"screen_low_bright": ""}`.
To write (`/set`) a value publish a message to topic `zigbee2mqtt/FRIENDLY_NAME/set` with payload `{"screen_low_bright": NEW_VALUE}`.
