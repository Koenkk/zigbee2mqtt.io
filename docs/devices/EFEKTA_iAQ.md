...
| Picture | ![Custom devices (DiY) EFEKTA_iAQ](https://www.zigbee2mqtt.io/images/devices/EFEKTA_iAQ.jpg) |
...
If value equals `ON` auto_brightness is ON, if `OFF` OFF.
...
If value equals `ON` long_chart_period is ON, if `OFF` OFF.
