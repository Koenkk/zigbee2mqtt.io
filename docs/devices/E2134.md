...
| Exposes | occupancy, illuminance, identify, battery, voltage, lux_value, timer |
...
### Illuminance sensor threshold (enum)
Controls whether the motion sensor triggers at all times or only when illuminance is low..
Value can be found in the published state on the `lux_value` property.
To read (`/get`) the value publish a message to topic `zigbee2mqtt/FRIENDLY_NAME/get` with payload `{"lux_value": ""}`.
To write (`/set`) a value publish a message to topic `zigbee2mqtt/FRIENDLY_NAME/set` with payload `{"lux_value": NEW_VALUE}`.
The possible values are: `always`, `only_when_dark`.

### On duration (numeric)
How long bound devices remain on after motion is detected..
...
The minimal value is `10` and the maximum value is `65534`.
