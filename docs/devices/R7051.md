...
### Warning (composite)
Can be set by publishing to `zigbee2mqtt/FRIENDLY_NAME/set_message` with payload `{"warning": {"mode": VALUE, "level": VALUE, "strobe_level": VALUE, "strobe": VALUE, "strobe_duty_cycle": VALUE, "duration": VALUE}}`
...
To write (`/set`) a value publish a message to topic `zigbee2mqtt/FRIENDLY_NAME/set_message` with payload `{"volume": NEW_VALUE}`.
The minimal value is `0` and the maximum value is `100`.
