...
To change the position publish a message to topic `zigbee2mqtt/FRIENDLY_NAME/set` with payload `{"position": VALUE}` where `VALUE` is a number between `0` and `100`. Alternatively publish a message to topic `zigbee2mqtt/FRIENDLY_NAME/set/position` with payload 'VALUE', where `VALUE` is a number between `0` and `100`.
