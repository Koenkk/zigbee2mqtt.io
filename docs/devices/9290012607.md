...
| Exposes | temperature, occupancy, battery, illuminance_lux, illuminance, motion_sensitivity, occupancy_timeout, led_indication, linkquality |
...
### LED indication
Enables or disables green LED indicator that can light up briefly when motion is detected.
The behaviour can be changed by publishing to `zigbee2mqtt/FRIENDLY_NAME/set`
`{"led_indication": VALUE}` where `VALUE` can be either `true` or `false`.
