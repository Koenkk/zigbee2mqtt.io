...
- Low click sensitivity:  When pressed lightly, the buttons produce audible clicks, without the device actually registering presses. **Press the buttons firmly** to ensure it generates events
...
The remaining percentage seems to be tuned for Alkalines (1.5V).
...
There's also [one report](https://github.com/Nerivec/silabs-firmware-builder/issues/102) that pairing is impossible with v9.0.0 pre-release Silabs firmware.
...
1. Optionally, [factory reset](#factory-reset) the remote *(to leave the network and clear bindings)*
...
4. First LED quickly blinks amber, indicating channel 1.  
   ...
   *You can bind multiple channels to the same device. And you can add multiple devices to one channel.*
...
6. When successful, the target device identifies, and the remote LEDs stop blinking.  
   The remote should now directly control the target, as described in [Action mapping](#action-mapping)
...
**All remotes directly control groups 21658, 21659, 21660** *(channels 1-3).* Group 21658 also conflicts with the [BILRESA dual-button](./E2489.md)!  
...
*This means that if you have multiple remotes in your network, you can't pair them to different devices/groups. Every remote will control each other's target. The workaround is using automations.*
