---
title: "Tuya TS020C control via MQTT"
description: "Integrate your Tuya TS020C via Zigbee2MQTT with whatever smart home infrastructure you are using without the vendor's bridge or gateway."
...
| Picture | ![Tuya TS020C](https://www.zigbee2mqtt.io/images/devices/TS020C.png) |
...
Value can be found in the published state on the `light_interval` property.
It's not possible to read (`/get`) this value.
To write (`/set`) a value publish a message to topic `zigbee2mqtt/FRIENDLY_NAME/set` with payload `{"light_interval": NEW_VALUE}`.
