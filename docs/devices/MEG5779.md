...
| Exposes | climate (local_temperature, occupied_heating_setpoint, unoccupied_heating_setpoint, occupied_cooling_setpoint, unoccupied_cooling_setpoint, system_mode, running_state, pi_heating_demand), max_heat_setpoint_limit, min_heat_setpoint_limit, max_cool_setpoint_limit, min_cool_setpoint_limit, display_brightness_active, display_brightness_inactive, display_active_timeout, temperature_display_mode, keypadLockout, thermostat_application, heating_fuel, heat_transfer_medium, heating_emitter |
...
### Climate 
This climate device supports the following features: `local_temperature`, `occupied_heating_setpoint`, `unoccupied_heating_setpoint`, `occupied_cooling_setpoint`, `unoccupied_cooling_setpoint`, `system_mode`, `running_state`, `pi_heating_demand`.
...
Value can be found in the published state on the `heating_fuel` property.
To read (`/get`) the value publish a message to topic `zigbee2mqtt/FRIENDLY_NAME/get` with payload `{"heating_fuel": ""}`.
To write (`/set`) a value publish a message to topic `zigbee2mqtt/FRIENDLY_NAME/set` with payload `{"heating_fuel": NEW_VALUE}`.
The possible values are: `electricity`, `gas`, `oil`, `solid_fuel`, `solar`, `community_heating`, `heat_pump`, `not_specified`.

### Heat transfer medium (enum)
Medium used to transfer heat..
Value can be found in the published state on the `heat_transfer_medium` property.
To read (`/get`) the value publish a message to topic `zigbee2mqtt/FRIENDLY_NAME/get` with payload `{"heat_transfer_medium": ""}`.
To write (`/set`) a value publish a message to topic `zigbee2mqtt/FRIENDLY_NAME/set` with payload `{"heat_transfer_medium": NEW_VALUE}`.
The possible values are: `nothing`, `hydronic`, `air`.
