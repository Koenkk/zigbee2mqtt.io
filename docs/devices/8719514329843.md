...
| Exposes | light (state, brightness), power_on_behavior, effect |
...
The round [LivingColors remote]() with color dial (e.g. from a Hue Iris lamp) can be used to factory reset many hue devices as well.

1. Get remote as close as possible to bulb
2. Press and hold Scene 1 (the one dot button) and On (I) button for five seconds, lamp will flicker and turn off, hold until the lamp is on again

This might also work for other hue devices.
