---
title: "AVATTO ME168 control via MQTT"
description: "Integrate your AVATTO ME168 via Zigbee2MQTT with whatever smart home infrastructure you are using without the vendor's bridge or gateway."
...
| Picture | ![AVATTO ME168](https://www.zigbee2mqtt.io/images/devices/ME168.jpg) |
...
- `system_mode`: Mode of this device. To control publish a message to topic `zigbee2mqtt/FRIENDLY_NAME/set` with payload `{"system_mode": VALUE}` where `VALUE` is one of: `auto`, `heat`, `off`. Reading (`/get`) this attribute is not possible. If value equals `heat` the device will start heating depending on the `current_heating_setpoint`, if `auto` the `current_heating_setpoint` is additionally set according to the schedules, if `off` the device will ignore the `current_heating_setpoint`.
