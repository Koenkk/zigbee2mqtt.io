...
All of the actions below can also be done via the WebUI.
...
To also provide a name for the scene (shown in e.g. the WebUI) send e.g. `{"scene_store": {"ID": 1, "name": "My scene"}}`.
...
```js
{
    "scene_add": {
        "ID": 3, // the SCENE_ID
        "name": "Chill scene", // optional: scene name, shown in e.g. the WebUI
