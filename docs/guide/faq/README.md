...
## How do I migrate from one adapter to another, including previously unsupported adapters (advanced users)?
...
This was successfully tested when migrating from Conbee 2 to Sonoff Dongle P. This _should_ work for other combinations, but you might want to wait for more feedback.
:::

1. __Backup your Zigbee2MQTT configuration and data.__
2. Make sure you are running the latest version of Zigbee2MQTT.
3. Stop Zigbee2MQTT.
4. Install the latest firmware on both your current adapter __and__ future adapter.
...
10. Restore the source coordinator backup to the target coordinator with `zigpy radio znp /dev/ttyUSB1 restore deconz-backup.json`.
    ...
    - The restore process may take several minutes. __DO NOT INTERRUPT__.
...
13. Open Z2M's `configuration.yaml` file:
    ...
    - This is located under `serial`. The important fields are `port` and `adapter`. Refer to [Z2M configuration](https://www.zigbee2mqtt.io/guide/configuration/adapter-settings.html) for details.
